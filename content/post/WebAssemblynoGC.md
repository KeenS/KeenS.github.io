...
* メモリは`grow_memory`で16KiB単位で増やせる
...
2種類の構造体を使います。それぞれ1ページ、16KiB使います。
C言語風に書くとこうです。
...
ここはページが16KiBアラインされていることを使います。

ページが16KiBアラインされているのでページのアドレスは下位14bitが0です。なので単にポインタの下位14bitをクリアしてあげると自分が所属しているページのアドレスが手に入るのです。
