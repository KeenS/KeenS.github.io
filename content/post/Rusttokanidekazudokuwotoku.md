...
この関数に空欄を適当に埋めたパズルを与えると、パズルに正解がなければ空欄にどんな数値を入れても `false` になりますし、、正解があるならなら1つは `true` となる空欄の埋め方があります。
そこでkaniの機能を使って空欄のあるパズルを用意して、 `assert!(!check(puzzle));` のように宣言すると「このpuzzleはどのような埋め方をしても正解にならない」という意味になり、kaniが反例を探してくれます。
