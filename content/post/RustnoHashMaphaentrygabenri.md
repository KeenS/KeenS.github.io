...
`Entry` に対して `or_insert` を呼ぶときは `Occupied` ならそこにある値を返せばいいですし、なければ `or_insert` の引数をその場に値を置けばいいです。既に場所が確定したあとなので、値を置くのは一瞬です。
...
因みにこのAPIを提供できるのはRustの借用/所有権システムのおかげです。他言語でこれをやると、 `Entry` を保持している間にその「場所」に別の値を上書かれてしまった場合に破滅します。
