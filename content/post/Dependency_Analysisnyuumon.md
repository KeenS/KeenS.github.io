...
一般に、プログラムを書き換えると結果が変わってしまうことがあります。
...
このプログラムを書き換えて1を足し込む順番を変えたらどうなるでしょう。
...
これらの違い、もちろん感覚では分かるかと思いますが、ちゃんと理路整然と説明できますか？
...
さて、先程までの例だと少し単純すぎるのでもうちょっと実用的な例を出しましょう。
以下のシンプルなプログラムを考えます。
...
この関数に全て1.0で埋まった128要素の配列（要するに`[1.0; 128]`）を渡して実行することにします。
...
最初のプログラムの例でいうと $\mathit{L1}\ \delta^f\ \mathit{L2}$ 、 $\mathit{L2}\ \delta^f\ \mathit{L2}$ （`v += 1` は `v = v + 1` なので）、 $\mathit{L2}\ \delta^f\ \mathit{L3}$ ですね。全てフロー依存から構成されています。
...
今回はループの **反復空間** （iteration space）が正規誘導変数のアフィン式で書けるようなループのみ扱うことにします。
